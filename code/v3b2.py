...
def spinner():
    ...
    while True:
        sys.stdout.write(next(spinner))  # spinner의 다음 문자 출력
        sys.stdout.flush()
        time.sleep(0.1)
        sys.stdout.write('\b')  # 커서를 한 칸 뒤로 이동
...
def get_upload_url(api_key):
    ...
    if response.status_code == 200:
        ...
        return None
#tqdm으로 업로드 진행률 표시해보기
def upload_file(api_key, file_path, upload_url):
    headers = {'x-apikey': api_key}

    with open(file_path, 'rb') as file:
        files = {'file': (os.path.basename(file_path), file)}
        with tqdm(desc="Uploading file", total=1, unit_scale=True) as pbar:
